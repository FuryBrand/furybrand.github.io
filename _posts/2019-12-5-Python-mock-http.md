...
通过`pip install -U Flask`来安装Flash。Flask是一个使用 Python 编写的轻量级 Web 应用框架。在[GitHub](https://github.com/pallets/flask/)上可以找到相当有用的信息，我这里就是一个小Demo吧。
...
主要实现了如下功能：
- 1.用Postman访问`/setReturn`地址，同时传入参数来设置接下来的返回值。
- 2.用Postman访问`/return`地址，即可得到上一部种传入的值。
